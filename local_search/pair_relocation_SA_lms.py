...
def main():
    ...
    for iter in range(MAX_ITER):
        ...
        for cand in cands:
            ...
            if j - i == 1:
                d_removal = c[route[i-1]][route[j+1]] - c[route[i-1]][cand]\
                      - c[cand][cand+N] - c[cand+N][route[j+1]]
            else:
                d_removal = c[route[i-1]][route[i+1]] - c[route[i-1]][cand] - c[cand][route[i+1]]\
                    + c[route[j-1]][route[j+1]] - c[route[j-1]][cand+N] - c[cand+N][route[j+1]]
            route.pop(j)
            route.pop(i)
            ...
            for i_prime in range(2*N - 1):
                if (i_prime <= max_cap_ids[-1]) and (route_max_cap + 1 > K):
                    continue
                d_insertion = c[route[i_prime]][cand] + c[cand][cand+N] + \
                    c[cand+N][route[i_prime+1]] - c[route[i_prime]][route[i_prime+1]]
            ...
            min_d_jp[2*N - 3] = (c[route[2*N - 2]][cand+N] +
                                c[cand+N][0] - c[route[2*N - 2]][0], 2*N - 2)
            for i_prime in range(2*N - 4, -1, -1):
                d = c[route[i_prime+1]][cand+N] + c[cand+N][route[i_prime+2]]\
                    - c[route[i_prime+1]][route[i_prime+2]]
            ...
            for i_prime in range(2*N - 2):
                ...
                if (result != -1 and j_prime < max_cap_ids[result]) or route_max_cap + 1 <= K:
                    d_insertion = c[route[i_prime]][cand] + c[cand][route[i_prime+1]]\
                        - c[route[i_prime]][route[i_prime+1]] + min_d_jp[i_prime][0]
                    if P(min_d_insertion, d_insertion, T) > random.random():
                        min_d_insertion = d_insertion
